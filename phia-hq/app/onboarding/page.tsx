...
export default function OnboardingPage() {
  ...
  async function handleSave() {
    ...
    // Tab 2 → wait for any pending uploads, save everything, toast, navigate
    setSaving(true);
    setUploadProgress(5);

    const toastId = toast.loading("Saving your style profile…", { duration: Infinity });

    // Wait for any in-flight uploads to finish
    if (pendingUploads.current.size > 0) {
      const interval = setInterval(() => {
        setUploadProgress((p) => Math.min(p + 3, 80));
      }, 300);
      await Promise.allSettled([...pendingUploads.current]);
    ...
    toast.success("Profile saved!", {
      id: toastId,
      description: "Phia is learning your style.",
      duration: 3000,
    });
  ...
  if (saving) return <SaveLoader uploadProgress={uploadProgress} />;
